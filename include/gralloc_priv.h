...
struct private_handle_t : public native_handle {
    ...
    struct private_handle_t {
...
#else
        /*
         * Sony cam HAL was compiled with this fd_metadata element
         * along with genlockHandle - which pushes size and
         * offset 4 bytes down.
         */
        ...
        int     flags;
        /*
         * Move bufferType up two positions in order to compensate for
         * size/offset of cam HAL being 4 bytes down.
         * bufferType is only used in stuff we compile so we can
         * move it around.
         */
        int     bufferType;
        int     size;
        int     offset;
        // bufferType WAS here previously.
        int     base;
#ifdef QCOM_BSP
        int     offset_metadata;
#endif
        ...
        private_handle_t(int fd, int size, int flags, int bufferType,
            ...
            fd_metadata(eFd),
#endif
            magic(sMagic),  flags(flags), bufferType(bufferType),
            size(size), offset(0), base(0),
#ifdef QCOM_BSP
